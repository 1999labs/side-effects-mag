...
5. **Don't edit `feed.json` or the README archive table.** Editors handle that on merge, which keeps PRs focused on the prose.
...
4. **Publish.** The piece goes live on merge, and `npx side-effects-mag read 00X` works within seconds via GitHub's raw CDN. The email send to subscribers usually follows within 24 hours.
